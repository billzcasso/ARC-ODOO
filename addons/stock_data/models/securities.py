...
class Securities(models.Model):
    ...
    @classmethod
    def _auto_create_security(cls, env, symbol):
        """
        Attempt to create security record from Gateway if missing.
        Tries all markets (HOSE, HNX, UPCOM) to find the symbol.
        """
        try:
            # 1. Check DB first (Bypass cache)
            # Use search_read for speed, though search is fine
            sec = env['ssi.securities'].sudo().search([('symbol', '=', symbol)], limit=1)
            if sec:
                cls._symbol_cache[symbol] = sec.id
                return sec.id
            
            # 2. Not in DB? Fetch from SSI Gateway
            # We need to manually instantiate gateway as we are in a classmethod
            api_config = env['ssi.api.config'].get_config()
            if not api_config:
                return None
                
            gateway = SSIGateway(api_config.consumer_id, api_config.consumer_secret, api_config.api_url)
            
            # Try each market to find details
            ...
            for market in ['HOSE', 'HNX', 'UPCOM']:
                try:
                    ...
                    if items:
                        ...
                        # Create and Commit immediately because we need this ID for the current streaming batch
                        new_sec = env['ssi.securities'].sudo().create(vals)
                        env.cr.commit()
                        
                        _logger.info("Auto-created security from stream: %s (%s)", symbol, market)
                        
                        # Update Cache
                        cls._symbol_cache[symbol] = new_sec.id
                        
                        # Trigger Hooks (like Fund Creation) manually if needed?
                        # create() method overrides already do this!
                        # But create() on new_sec called .create().
                        # Verify create() override calls invalidate_symbol_cache()
                        # And verify stock_data_extension create() calls _propagate_to_fund_certificate()
                        
                        return new_sec.id
                except Exception as ex:
                    # Ignore market mismatch errors
                    continue
                    
        except Exception as e:
            _logger.error("Failed to auto-create security %s: %s", symbol, e)
    ...
    def _process_streaming_message(self, message):
        ...
        try:
            ...
            for item in items:
                ...
                if not security_id:
                    # Logic to auto-create if missing (User expectation)
                    security_id = self._auto_create_security(env, symbol)
                    if not security_id:
                        continue
